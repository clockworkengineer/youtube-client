...
pub async fn execute_subscribe(ctx: &CliContext, channel_id: String) -> anyhow::Result<()> {
...
pub async fn execute_unsubscribe(ctx: &CliContext, subscription_id: String) -> anyhow::Result<()> {
    let client = ctx.get_client().await?;
