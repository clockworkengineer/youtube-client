        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            match current_view {
                                    ...
                                    .show(ui, |ui| {
                                        for sub in subs {
                                            ui.push_id(&sub.channel_id, |ui| {
                                                let texture = get_or_fetch_thumbnail(&self.state, &self.http_client, ctx, &sub.channel_id, &sub.thumbnail_url);
                        ...
                        Some(Ok(vids)) => {
                            if vids.is_empty() {
                                ...
                                egui::ScrollArea::vertical()
                                    ...
                                    .show(ui, |ui| {
                                        for video in vids {
                                            draw_video_card_with_dismiss(&self.state, &self.http_client, &self.audio_tx, ui, ctx, video, &mut action, true);
                                        }
                                    });
                            }
                        }
                        ...
                        Some(Ok(vids)) => {
                            if vids.is_empty() {
                                ...
                                egui::ScrollArea::vertical()
                                    ...
                                    .show(ui, |ui| {
                                        for video in vids {
                                            draw_video_card(&self.state, &self.http_client, &self.audio_tx, ui, ctx, &video, &mut action);
                                        }
                                    });
                            }
                        }
                                    ...
                                    .show(ui, |ui| {
                                        for list in lists {
                                            ui.push_id(&list.id, |ui| {
                                                let texture = get_or_fetch_thumbnail(&self.state, &self.http_client, ctx, &list.id, &list.thumbnail_url);
                ...
                View::VideoDetails { video, comments } => {
                    ...
                    ui.horizontal(|ui| {
                        let texture = get_or_fetch_thumbnail(&self.state, &self.http_client, ctx, &video.id, &video.thumbnail_url);
                        if let Some(tex) = &texture {
